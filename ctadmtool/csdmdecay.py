#! /usr/bin/env python
# ==========================================================================
# Perform Analysis for DM models: Decay
#
...
class csdmdecay(ctools.csobservation):
    """
        This class performs a search for gamma-ray signals
        induced by decay of DM particles in a target
    ...
    def __init__(self,*argv):
        ...
        self._masses      = []
        self._dfactor     = 0.0
        self._lifetimeref = 0.0
        self._lifetimeLL  = 0.0
        self._deltats     = []
    ...
    def __getstate__(self):
        ...
        state = {'base'       : ctools.csobservation.__getstate__(self),
                ...
                'lifetimeref' : self._lifetimeref,
                'lifetimeLL'  : self._lifetimeLL,
    ...
    def __setstate__(self, state):
        ...
        self._lifetimeref = state['lifetimeref']
        self._lifetimeLL  = state['lifetimeLL']
    ...
    def _get_parameters(self):
        ...
        self._lifetimeref = math.pow(10.,self['loglifetime'].real())
    ...
    def _gen_dmmodel(self,i):
        ...
        Return
        ------
        GModel for a dark matter decay
        ...
        The normalization is computed using (decay):
        ...
        dfactor   = self._dfactor
        lifetime  = self._lifetimeref
        fluxnorm  = dfactor/(4.*gammalib.pi*dmmass*lifetime)
        fluxnorm *= 1.e-3
        #scale     = 1.0
        #fluxnorm /= scale
        ...
        dminterp = dmspectrum(dmmass,emin,thisemax,channel,
            redshift,process='decay',eblmod=eblmodel,
    ...
    def _fit_mass_point(self,i):
        ...
        header = 'Fit the DM model for decay'
        ...
        msg = '{:.3e} (s)'.format(self._lifetimeref)
        self._log_value(gammalib.TERSE,'Ref. Lifetime',msg)
        msg = '{:.3e} (GeV/cm**2)'.format(self._dfactor)
        self._log_value(gammalib.TERSE,'D factor',msg)
        ...
        result = {'e_min'        : gemin.TeV(),
                  ...
                  'ulimit'       : 0.0,
                  'sc_factor'    : 0.0,
                  'lifetime_ref' : self._lifetimeref,
                  'lifetime_lim' : 0.0}
        ...
        #   NOTE:
        #   The part about increasing the accuracy of the
        #   likelihood is not needed, as we are using the
        #   TS profile to get the UL to the flux
        like                  = ctools.ctlike(obssim)
        like['edisp']         = self['edisp'].boolean()
        like['like_accuracy'] = 1.e-5
        like['accept_dec']    = 10
        ...
        if logL0 != 0.0:
            ...
            if self['calc_ulim'].boolean():
                ...
                self._lifetimeLL       = self._lifetimeref/scale
                result['lifetime_lim'] = self._lifetimeLL
                ...
                if len(svmsg) > 0:
                    ...
                    msg  = 'LL to decay lifetime is: %.3e' % (lifetimelim)
                    msg += ' s'
    ...
    def _create_fits(self,results):
        ...
        #   Decay  --> Lifetime
        paroi_lim = gammalib.GFitsTableDoubleCol('LLLifetime' ,nrows)
        ...
        for i,result in enumerate(results):
            ...
            colnm = 'Correlation (Mass {})'.format(i)
    ...
    def publish(self,name=''):
        ...
        #   Log file
        self._log_value(gammalib.NORMAL,'DM decay name',user_name)

        #   Publishing
        self._fits.publish('DM Decay',user_name)
    ...
    def lifetime_ref(self):
        """
        Return Reference Cross-section value
        """
        #   Return
        return self._lifetimeref

    def lifetime_ll(self):
        """
        Return Reference Cross-section value
        """
        #   Return
        return self._lifetimeLL
