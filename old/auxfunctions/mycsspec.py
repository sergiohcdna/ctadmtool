...
class csspec(ctools.csobservation):
    ...
    def _fit_model(self):
        ...
        lognorms = np.logspace( -3.0 , 3.0 , 50 )
        ...
        for i in range(spectrum.nodes()):
            ...
            if self['calc_ts'].boolean():
                ...
                self._log_header3(gammalib.EXPLICIT, 'Performing scan over Normalization')

                thisTS = []
                for norm in lognorms :
                    thisobs = like.obs().copy()
                    pars = thisobs.models()[ self[ 'srcname' ].string() ].spectral()[ i * 2 + 1 ]
                    pars.autoscale()
                    pars.factor_min( 1.0e-10 )
                    pars.factor_max( 1.0e+05 )
                    pars.factor_value( norm )
                    pars.autoscale()
                    pars.fix()
            ...
            if self['calc_ts'].boolean() and result['TS'] > 0.0:
                ...
                thischain = ''
                for valor in thisTS :
                    thischain += 'Scan Norm with TS: %.3e' % ( valor )
            self._log_value(gammalib.TERSE, 'Bin '+str(i+1), value)
            self._log_value( gammalib.TERSE , 'Bin' + str( i + 1 ) , thischain )
